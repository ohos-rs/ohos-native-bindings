...
use ohos_display_soloist_binding::{
    DisplaySoloist, DisplaySoloist_ExpectedRateRange,
};
...
static EXCLUSIVE_SOLOIST: LazyLock<DisplaySoloist> =
    LazyLock::new(|| DisplaySoloist::new(true));
...
pub fn set_frame_rate(min: i32, max: i32, expected: i32) {
    DISPLAY_SOLOIST.set_frame_rate(DisplaySoloist_ExpectedRateRange {
        min,
        max,
        expected,
    });
